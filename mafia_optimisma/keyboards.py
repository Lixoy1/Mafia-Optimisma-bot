...
def join_keyboard(chat_id: int) -> InlineKeyboardMarkup:
    """Registration message keyboard: only the join button, as requested."""
...
def night_action_keyboard(game: GameState, player: PlayerState) -> InlineKeyboardMarkup | None:
    ...
    def target_buttons(action_name: str, include_self: bool = False, prefix: str = "", exclude_checked: bool = False) -> list[InlineKeyboardButton]:
        res = []
        for t in targets:
            ...
            if exclude_checked and t.user_id in player.checked_ids:
                continue
            res.append(InlineKeyboardButton(text=f"{prefix}{t.name[:24]}", callback_data=f"n:{game.chat_id}:{action_name}:{t.user_id}"))
        return res

    if action in {"mafia_kill_leader", "mafia_kill_backup"}:
        rows = chunk_buttons(target_buttons("mafia_kill", prefix="🔪 "), 2)
    elif action in {"yakuza_kill_leader", "yakuza_kill_backup"}:
        rows = chunk_buttons(target_buttons("yakuza_kill", prefix="🎴 "), 2)
    elif action == "heal":
        ...
        for t in targets:
            if t.user_id == player.user_id and player.self_heals_used >= 1:
                continue
            buttons.append(InlineKeyboardButton(text=f"🩺 {t.name[:24]}", callback_data=f"n:{game.chat_id}:heal:{t.user_id}"))
        rows = chunk_buttons(buttons, 2)
    elif action == "check_or_shoot":
        # Two clearly separated action variants: check or shoot.
        if game.mode != "clans":
            ...
            if check_buttons:
                ...
                rows.append([InlineKeyboardButton(text="✅ Все доступные игроки уже проверены", callback_data=f"noop:{game.chat_id}:checked_all")])
        ...
        rows = chunk_buttons(target_buttons("block", prefix="💋 "), 2)
        ...
        rows = chunk_buttons(target_buttons(action, prefix=prefix_map.get(action, "")), 2)
    elif action == "compare_clans":
        rows = chunk_buttons(target_buttons("report1", prefix="🗞 "), 2)
    elif action == "swap_roles":
        rows = chunk_buttons(target_buttons("swap1", include_self=True, prefix="🃏 "), 2)
    else:
        return None
