...
async def _start_mode(message: Message, mode: str):
    ...
    await message.answer(
        f"🎲 Регистрация началась. Режим: {mode_line(mode)}\n\n"
        "Игроки могут войти через кнопку в закрепе или команду /join.",
        reply_markup=join_keyboard(message.chat.id),
    )
...
async def extend_registration(message: Message):
    ...
    await engine.update_registration_message(message.bot, game)
    await message.answer(
        "⏱ Регистрация продлена на 30 секунд. Успевайте войти в город!",
        reply_markup=join_keyboard(message.chat.id),
    )
...
async def call_players(message: Message):
    ...
    mentions = [call_mention(u) for u in users if not message.from_user or u.get("user_id") != message.from_user.id]
    mention_text = " ".join(mentions[:50]) if mentions else "Пока некого звать: игроки должны хотя бы раз написать боту/войти через /join."
    await message.answer(
        pick(GLOBAL["call_start"], bot_name=bot_name) + "\n\n" + mention_text,
        reply_markup=join_keyboard(message.chat.id),
    )
