...
def generate_roles(mode: str, count: int) -> list[str]:
    """Role distribution tuned to the original rules and early-game balance.

    Classic mode must not give Ищейка at 4 players.
    For 4 players the exact pack is: 2 Оптимиста, 1 Хирург, 1 Карлеоне.
    Ищейка appears from 6 players in classic mode.
    """
    if mode == "clans":
    ...
    if mode == "classic":
        # Минимальная рабочая классика:
        # 4 игрока = Карлеоне + Хирург + 2 Оптимиста.
        roles += ["surgeon"]
        if count >= 6:
            roles += ["tracker"]
        # Дополнительные роли включаются позже, не ломая малые игры.
    ...
    # Remove duplicates while keeping order for roles that were added by thresholds too early/again.
...
class GameEngine:
    ...
    async def begin_registration(self, bot: Bot, game: GameState) -> None:
        """Send pinned registration message and start the 30-second auto-start timer."""
        await self.public_registration_message(bot, game)
        self.schedule_registration(bot, game)

    def schedule_registration(self, bot: Bot, game: GameState, seconds: int | None = None) -> None:
        """Start/restart registration timer. /extend uses this to add another 30 seconds."""
        delay = seconds or self.settings.registration_seconds
        self._schedule(game.chat_id, delay, self.auto_start_registration(bot, game))

    async def auto_start_registration(self, bot: Bot, game: GameState) -> None:
        """Automatically start game after registration timer, or warn if not enough players."""
        ...
        if len(game.players) < min_players:
            await bot.send_message(
                game.chat_id,
                f"⏳ Регистрация завершена, но игроков не хватает.\n"
                f"Для режима **{MODES[game.mode]['emoji']} {MODES[game.mode]['name']}** нужно минимум {min_players}.\n"
                f"Сейчас в игре: {len(game.players)}.\n\n"
                "Нажми кнопку «➕ Присоединиться» в закрепе или продли регистрацию командой /extend.",
    ...
    async def start_game(self, bot: Bot, game: GameState) -> None:
        ...
        if len(game.players) < min_players:
            await bot.send_message(
                game.chat_id,
                f"Для режима **{MODES[game.mode]['emoji']} {MODES[game.mode]['name']}** нужно минимум {min_players} игрока(ов). "
                f"Сейчас: {len(game.players)}. Продлите регистрацию командой /extend.",
                reply_markup=join_keyboard(game.chat_id),
            )
            return
    ...
    async def resolve_night(self, bot: Bot, game: GameState) -> tuple[list[tuple[PlayerState, str]], list[str]]:
        ...
        # Хирург может вылечить себя только один раз за игру.
        # Если повторно нажал себя, действие игнорируется.
        ...
        for a in effective_actions:
            ...
            if a.action_type in {"check", "mafia_role_check"} and target:
                ...
                if a.action_type == "check" and actor.role_key == "tracker":
    ...
    async def start_voting(self, bot: Bot, game: GameState) -> None:
        ...
        await bot.send_message(
            game.chat_id,
            pick(GLOBAL["voting_start"], seconds=self.settings.voting_seconds) + "\n\nГолосование проходит в личных сообщениях с ботом.",
            reply_markup=open_bot_keyboard(me.username),
        )
        for p in game.alive_players():
            if p.silenced:
    ...
    def registration_text(self, game: GameState) -> str:
        ...
        lines = [
            ...
            "",
            f"⏱ До автостарта: {self.settings.registration_seconds} секунд.",
            "",
            "👥 Присоединились:",
        ]
        if game.players:
            for i, p in enumerate(game.players.values(), start=1):
                tag = f"@{p.username}" if p.username else p.name
                lines.append(f"{i}. {tag}")
        else:
            lines.append("пока никого нет")
        lines.append("")
        lines.append(f"Всего игроков: {len(game.players)}")
        return "\n".join(lines)
    ...
    async def update_registration_message(self, bot: Bot, game: GameState) -> None:
        ...
        try:
            ...
            # Message may be unchanged, too old, deleted, or not editable. Game should continue.
            pass
