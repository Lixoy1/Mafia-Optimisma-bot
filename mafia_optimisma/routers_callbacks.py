...
async def cb_join(callback: CallbackQuery):
    ...
    # Telegram не разрешает боту первым писать пользователю.
    # Поэтому перед добавлением проверяем, что ЛС с ботом открыты.
    try:
        await callback.bot.send_message(user.id, "🙂 ЛС с ботом открыты. Теперь ты сможешь получить роль и ночные кнопки.")
    except Exception:
        await callback.answer("Сначала открой ЛС с ботом и нажми /start, потом возвращайся и жми «Присоединиться».", show_alert=True)
        return
...
async def cb_pm(callback: CallbackQuery):
    ...
    if callback.data == "pm:profile":
        ...
        if game and game.phase in {Phase.NIGHT, Phase.DISCUSSION, Phase.VOTING}:
            await callback.message.answer("🛒 Магазин недоступен во время запущенной игры. Покупки можно делать до старта или после окончания партии.")
        else:
