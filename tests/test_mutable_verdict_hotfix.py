...
class MutableVerdictTests(unittest.TestCase):
    def test_verdict_keyboard_has_execute_pardon_and_abstain(self):
        ...
        self.assertIn("👍 Казнить", labels)
        self.assertIn("👎 Помиловать", labels)
        self.assertIn("🤍 Воздержаться", labels)
        self.assertIn("verdict:abc123:-1001:3:abstain", data)
    ...
    def test_callback_contract_allows_replacing_previous_choice(self):
        source = (ROOT / "mafia_optimisma" / "routers_callbacks.py").read_text(encoding="utf-8")
        section = source.split('@router.callback_query(F.data.startswith("verdict:"))', 1)[1]
        section = section.split('@router.callback_query(F.data.startswith("bomb:"))', 1)[0]
        self.assertIn('{"yes", "no", "abstain"}', section)
        self.assertIn('game.verdict_votes[voter.user_id] = current', section)
        self.assertIn('previous = game.verdict_votes.get(voter.user_id)', section)
        self.assertIn('Решение изменено:', section)
        self.assertNotIn('Твой голос уже принят.', section)

    def test_verdict_result_counts_abstain_separately(self):
    ...
    def test_non_vote_remains_different_from_explicit_abstain(self):
